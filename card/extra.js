    ...
    huogong: {
      ...
      async content(event, trigger, player) {
        ...
        event.chooseToDiscard ??= async (event2, player2, target2) => {
          const discardPostion = event2.discardPostion === void 0 ? "h" : event2.discardPostion;
          const cards2 = event2.cards2;
          const filterDiscard = event2.filterDiscard === void 0 ? { suit: get.suit(cards2[0]) } : event2.filterDiscard;
          const result3 = await player2.chooseToDiscard(discardPostion, filterDiscard).set("ai", (card) => {
